...
2. CBF-MPPI (Approach A: CBF 비용 페널티)
3. Shield-MPPI (Approach C: Shielded Rollout)

Usage:
    python shield_mppi_demo.py --live
    python shield_mppi_demo.py --live --duration 15
    python shield_mppi_demo.py --duration 12 --seed 42
"""
...
def create_controllers(common_kwargs, cbf_kwargs):
    ...
    return {
        "vanilla": {
            "model": vanilla_model, "params": vanilla_params,
            "controller": vanilla_ctrl,
            "sim": Simulator(vanilla_model, vanilla_ctrl, dt),
        },
        "cbf": {
            "model": cbf_model, "params": cbf_params,
            "controller": cbf_ctrl,
            "sim": Simulator(cbf_model, cbf_ctrl, dt),
        },
        "shield": {
            "model": shield_model, "params": shield_params,
            "controller": shield_ctrl,
...
def run_live(common_kwargs, cbf_kwargs, trajectory_fn, duration):
    ...
    fig.suptitle(
        "Shield-MPPI Live: Vanilla vs CBF vs Shield",
    ...
    for ox, oy, r in OBSTACLES:
        ...
        ax_xy.add_patch(plt.Circle(
            (ox, oy), r + 0.1, color="red", alpha=0.1, linestyle="--", fill=False
        ))
    ...
    def update(frame):
        ...
        for key in ctrls:
            ...
            data[key]["ref"].append(ref_pt.copy())
        ...
        time_text.set_text(
            f"t={t_now:.1f}s | "
            f"RMSE  Vanilla:{v_rmse:.3f}m  CBF:{c_rmse:.3f}m  Shield:{s_rmse:.3f}m"
        )
    ...
    for key in ctrls:
        ...
        print(f"  {key:>8s}: RMSE={rmse:.4f}m, Min dist={min_d:.4f}m")
...
def run_batch(common_kwargs, cbf_kwargs, trajectory_fn, duration, seed):
    ...
    for i, (key, label) in enumerate([
        ...
        results[key] = {"history": history, "metrics": metrics}

    # 메트릭 출력
    print("\n")
    for key, label in [("vanilla", "Vanilla MPPI"), ("cbf", "CBF-MPPI"), ("shield", "Shield-MPPI")]:
    ...
    print("=" * 60)
    print("CBF-MPPI Statistics".center(60))
    print("=" * 60)
    print(f"  Safety rate:      {cbf_stats['safety_rate']:.2%}")
    print(f"  Min min barrier:  {cbf_stats['min_min_barrier']:.4f}")
    print("=" * 60)
    print("Shield-MPPI Statistics".center(60))
    print("=" * 60)
    print(f"  Safety rate:            {shield_cbf_stats['safety_rate']:.2%}")
    print(f"  Mean intervention rate: {shield_stats['mean_intervention_rate']:.2%}")
    print(f"  Mean vel reduction:     {shield_stats['mean_vel_reduction']:.4f}")
    print("=" * 60 + "\n")
    ...
    fig.suptitle(
        "Shield-MPPI Comparison: Vanilla vs CBF vs Shield",
    ...
    for ox, oy, r in OBSTACLES:
        ...
        ax.add_patch(plt.Circle((ox, oy), r + 0.1, color="red", alpha=0.1, linestyle="--", fill=False))
        ax.plot(ox, oy, "rx", markersize=8)
    ...
    # [2,0] 최소 장애물 거리
    ax = axes[2, 0]
    v_md = compute_min_distances(v_states, OBSTACLES)
    c_md = compute_min_distances(c_states, OBSTACLES)
    s_md = compute_min_distances(s_states, OBSTACLES)
    ax.plot(times, v_md, color=COLORS["vanilla"], label="Vanilla", linewidth=2)
    ax.plot(times, c_md, color=COLORS["cbf"], label="CBF-MPPI", linewidth=2)
    ax.plot(times, s_md, color=COLORS["shield"], label="Shield-MPPI", linewidth=2)
    ...
    ax.set_title("Min Distance to Obstacle"); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # [2,1] 요약
    ax = axes[2, 1]; ax.axis("off")
    vm, cm, sm = results["vanilla"]["metrics"], results["cbf"]["metrics"], results["shield"]["metrics"]
    summary = f"""
    +------------------------------------------------+
    |           Comparison Summary                    |
    +------------------------------------------------+
    |  Vanilla:  RMSE={vm['position_rmse']:.4f}m  Time={vm['mean_solve_time']:.1f}ms  |
    |            MinDist={np.min(v_md):.4f}m                 |
    ...
    +------------------------------------------------+
    |  Obstacles: {len(OBSTACLES)}, Duration: {duration:.0f}s, Seed: {seed}         |
    +------------------------------------------------+
    """
    ...
    output_path = "plots/shield_mppi_comparison.png"
...
def main():
    parser = argparse.ArgumentParser(description="Shield-MPPI Comparison Demo")
    ...
    print("\n" + "=" * 80)
    print("Shield-MPPI Comparison Demo".center(80))
